...
test.beforeEach('cleanup database', t => BeneficiariesModel.deleteMany());
...
test('(200) must succeed on creating beneficiaries and return the newly created doc', async t => {
  ...
  ['id', 'createdAt', 'updatedAt'].forEach(field => t.truthy(createdBeneficiary[field]));
  ['createdAt', 'updatedAt'].forEach(field => t.truthy(dayjs(createdBeneficiary[field]).isValid()));
  t.deepEqual(createdBeneficiary.dependents, []);
  t.assert(createdBeneficiary.dependentsCount === 0);
  Object.keys(beneficiaryPayload)
    .forEach(key => t.assert(createdBeneficiary[key] === beneficiaryPayload[key]));
});

// Unhappy path tests
[ 'birthDate', 'name', 'cpf', 'rg' ].forEach(field => {
  test(`(500) must fail on creating an beneficiary as field "${field}" is empty`, async t => {
    ...
    .catch(error => {
      const { validator, ...err } = isRequiredValidator(field)(beneficiaryPayload);
      t.assert(error.response.statusCode == 500);
      t.deepEqual(error.response.body, translate.error(err, LOCALE, beneficiaryPayload));
    })
  });
});
