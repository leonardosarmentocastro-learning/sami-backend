...
const connectedOnURI = isProduction
	? ''
	: `URI: "${textSecondary(CONNECTION_STRING)}"`;

exports.getErrorMessageForDatabaseConnection = (error) => {
	const message = [
		[
			// Line 1
			errorBg.white('  ERROR  '),
...
exports.getInfoMessageForRetryingToConnect = (
	retries,
	retryCount,
	retryInSeconds
) => {
	const message = [
		...
		`${textPrimary('Failed to connect to database. Retry')} ${textSecondary(
...
exports.getSuccessMessageForDatabaseConnection = () => {
	const message = [
		...
		`${textPrimary(
			'Connection to MongoDB was stablished.'
		)} ${connectedOnURI}`.trim(),
	].join(' ');
