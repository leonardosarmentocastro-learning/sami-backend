...
const $translations = ({ __readdirSync, __readFileSync } = DEFAULT) => ({
	getAvailableLanguages() {
		return this.getTranslationFileNames()
			...
			.reduce((availableLanguages, translationFileName) => {
				const [language] = translationFileName.split(
	...
	load() {
		return this.getAvailableLanguages().reduce((translationFiles, language) => {
