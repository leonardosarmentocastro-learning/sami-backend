...
test('validator must return "false" when "field" is already in use by another user', async (t) => {
	...
	t.false(await isAlreadyInUseValidator(field)(userDoc).validator());
	t.true(await isAlreadyInUseValidator(`not_${field}`)(userDoc).validator());
});
...
test('validator must return "true" when "field" is not in use by another user but by himself', async (t) => {
	...
	t.true(await isAlreadyInUseValidator(field)(userDoc).validator());
});
