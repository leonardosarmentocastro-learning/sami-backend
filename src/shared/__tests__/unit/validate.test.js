...
const { isRequiredValidator } = require('../../validators');
...
test('must return "code", "field", "message" fields of the first error it faces', async (t) => {
	...
	const { validator: validator1, ...error1 } = isRequiredValidator('email')(
		userDoc1
	);
	t.deepEqual(await sharedValidate(constraints, userDoc1), error1);

	const userDoc2 = { email: 'email@domain.com', username: '' };
	const { validator: validator2, ...error2 } = isRequiredValidator('username')(
		userDoc2
	);
