...
test('[sharedSanitizer] must remove not settable fields from schema', (t) => {
	...
	const schema = SCHEMA_NOT_SETTABLE_FIELDS.reduce(
		(accumulator, field) => ({ ...accumulator, [field]: 'some value' }),
		{}
	);

	t.deepEqual(sharedSanitizer({ ...schema, settableField }), { settableField });
